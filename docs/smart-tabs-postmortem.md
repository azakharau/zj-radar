...
- **Timer cost must stay cadence-gated.** `Fast` (8 Hz) is armed only for
  animation or unsettled domain work. A named quiet rail uses `Idle` (1 Hz) for
  one shared presence-directory read; an unnamed quiet rail disarms. No
  cadence does work proportional to pane count. Keep it that way:
  never let a timer tick fan out into N host calls.
