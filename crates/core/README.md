...
which aggregates per-pane AI-agent status into a `{pipe_agents}` widget in
your zjstatus Zellij status bar.
...
(the host CLI and the `zj-radar-agents` wasm plugin) depend on it, so the wire
contract can never drift between them.

Depend on this crate if you are writing a **producer**: a tool that broadcasts
`zj_radar.status.v1` payloads for the aggregator to display. Build a
...
Everything else — the aggregator plugin, the CLI, setup instructions — lives
in the [main repository](https://github.com/marktoda/zj-radar).
